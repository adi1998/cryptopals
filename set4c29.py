...
class Sha1Hash(object):
    ...
    def change_state(self,new_hash,l):
        #for i in  self._h:print( hex(i))
        #print (new_hash)
        self._h=[int(new_hash[i*8:i*8+8],16) for i in xrange(5)]
        self._unprocessed=''
    ...
    def _produce_digest(self):
        ...
        #print(repr(message),len(message))
...
mac=Sha1Hash().update(key+message)

h1=mac.hexdigest()
#print(h1)

forge=Sha1Hash().change_state(h1,calc_bytes('b'*keylen+message))

forge.update(addend)
print(forge.hexdigest())

new_message=message+forge_padding('b'*keylen+message)+addend
check=Sha1Hash().update(key+new_message).hexdigest()
print(check)
